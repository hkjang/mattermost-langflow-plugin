...
func (p *Plugin) OnActivate() error {
	...
	if err := p.ensureBots(); err != nil {
		p.API.LogError("Failed to ensure Langflow bots during activation", "error", err)
	}
...
func (p *Plugin) MessageHasBeenPosted(_ *plugin.Context, post *model.Post) {
	...
	go func() {
		if err := p.handlePostedMessage(post); err != nil {
			p.API.LogError("Failed to process Langflow post trigger", "error", err, "post_id", post.Id)
		}
	}()
}
...
func buildBotPromptMessage(bot BotDefinition) string {
	lines := []string{
		fmt.Sprintf("`@%s` is ready to run Langflow flow `%s`.", bot.Username, bot.FlowID),
		"",
		"Try one of these:",
	}
